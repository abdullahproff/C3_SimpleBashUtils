...
void bn_result(int count, int enter, int str_count, int enter_count,
               char current, int *str_count_b, flag_type **flag) {
  if ((*flag)->b && enter == 1) {
    if (enter == 1 && current != '\n') {
...
void svET_result(char current, int enter_count, int *str_count,
                 flag_type **flag) {
  if ((*flag)->E && current == '\n') {
    if ((*flag)->b || (*flag)->s) {
      if (enter_count < 3) {
